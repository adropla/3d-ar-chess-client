...
window.DemoUtils = {
  ...
  /**
   * Loads all the OBJ models with an MTL material applied.
   * Returns array of THREE.Group objects containing the mesh.
   *
   * @param {array} models
   * @return [Promise<THREE.Group>]
   */
  loadAllModels(models) {
    ...
    const promisesModels = []
    for (let index = 0; index < models.length; index++) {
      const { mtlURL } = models[index]
      const { objURL } = models[index]
      ...
      // return a Promise and resolve or reject based off of the asset
      // downloading.
      const newModelPromise = new Promise((resolve, reject) => {
        mtlLoader.load(
          mtlURL,
          (materialCreator) => {
            // We have our material package parsed from the .mtl file.
      ...
      promisesModels.push(newModelPromise)
    }

    return Promise.all(promisesModels)
  },
...
class Reticle extends THREE.Object3D {
  ...
  constructor(xrSession, camera) {
    ...
    console.log('Reticle constructor')
  ...
  async update(frameOfRef) {
    ...
    if (hits.length) {
      ...
      this.icon.material.color.set(this.icon.ready ? 0x24e020 : 0xffffff)
      this.ring.material.color.set(this.icon.ready ? 0x24e020 : 0xffffff)
