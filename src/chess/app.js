...
class App {
  ...
  loadModels() {
    ...
    DemoUtils.loadAllModels(models).then((modelArrays) => {
  ...
  onDocumentMouseDown = (event) => {
    ...
    if (intersect) {
      if (this.selectedPiece) {
        ...
        this.currentIntersectFace =
          intersect.faceIndex % 2 === 0
            ? intersect.faceIndex
            : intersect.faceIndex - 1
        this.selectedPiece.oldFaceIndex =
          this.selectedPiece.newFaceIndex || null
        ...
        if (this.selectedPiece) {
          ...
          this.tweenRotateSelectedPiece = new TWEEN.Tween(
            ...
            .to(
              {
                ...
                y: Math.PI * 2,
                z: this.selectedPiece.rotation.x,
              },
              2000,
            )
            .repeat(Infinity)
  ...
  validMove() {
    ...
    if (this.piecePositions[this.currentIntersectFace]) {
      const capturedPieceInfo = this.getPieceInfo(
        this.piecePositions[this.currentIntersectFace],
      )
  ...
  getSelectedPiece(intersects) {
    const intersectedPiece = intersects.find((model) =>
      model.object.parent.name.includes('Piece'),
    )
  ...
  capturePiece() {
    if (this.piecePositions[this.currentIntersectFace]) {
      ...
      const dropPiece = this.pieces[capturedPieceInfo.color][
        capturedPieceInfo.type
      ].splice(capturedPieceInfo.index, 1)[0]
      const dropTween = new TWEEN.Tween(dropPiece.position)
        .to(
          {
            x: dropPiece.position.x,
            y: dropPiece.position.y - 0.1,
            z: dropPiece.position.z,
          },
          2000,
        )
        .easing(TWEEN.Easing.Back.InOut)

      const rotateTween = new TWEEN.Tween(dropPiece.rotation).to(
        { x: Math.PI / 2, y: dropPiece.position.y, z: dropPiece.position.z },
        1000,
      )
  ...
  onClick = async (e) => {
    ...
    if (this.reticle.visible) {
      ...
      for (let i = 0; i <= 7; i++) {
        ...
        this.pieces.white.pawn[i].position.set(
          pawnPosition.x,
          pawnPosition.y,
          pawnPosition.z,
        )
        ...
        this.pieces.black.pawn[i].position.set(
          pawnPosition.x,
          pawnPosition.y,
          pawnPosition.z,
        )
      ...
      for (let i = 0; i < 2; i++) {
        ...
        this.pieces.white.rook[i].position.set(
          rookPosition.x,
          rookPosition.y,
          rookPosition.z,
        )
        ...
        this.pieces.black.rook[i].position.set(
          rookPosition.x,
          rookPosition.y,
          rookPosition.z,
        )
      ...
      for (let i = 0; i < 2; i++) {
        ...
        this.pieces.white.knight[i].position.set(
          knightPosition.x,
          knightPosition.y,
          knightPosition.z,
        )
        ...
        this.pieces.black.knight[i].position.set(
          knightPosition.x,
          knightPosition.y,
          knightPosition.z,
        )
      ...
      for (let i = 0; i < 2; i++) {
        ...
        this.pieces.white.bishop[i].position.set(
          bishopPosition.x,
          bishopPosition.y,
          bishopPosition.z,
        )
        ...
        this.pieces.black.bishop[i].position.set(
          bishopPosition.x,
          bishopPosition.y,
          bishopPosition.z,
        )
      ...
      this.pieces.white.king[0].position.set(
        kingPosition.x,
        kingPosition.y,
        kingPosition.z,
      )
      ...
      this.pieces.black.king[0].position.set(
        kingPosition.x,
        kingPosition.y,
        kingPosition.z,
      )
      ...
      /* Set queen */
      const queen = new THREE.Box3().setFromObject(this.pieces.white.queen[0])
      ...
      this.pieces.white.queen[0].position.set(
        queenPosition.x,
        queenPosition.y,
        queenPosition.z,
      )
      ...
      this.pieces.black.queen[0].position.set(
        queenPosition.x,
        queenPosition.y,
        queenPosition.z,
      )
  ...
  createChessBoard() {
    ...
    const chessBoardMesh = new THREE.Mesh(
      cbgeometry,
  ...
  getPieceInfo(piece) {
    ...
    return {
      ...
      index: parseInt(pieceNameSplit[3], 10),
  ...
  onXRFrame = (time, frame) => {
    ...
    const { framebuffer } = this.xrSession.renderState.baseLayer
